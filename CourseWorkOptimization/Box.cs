...
namespace CourseWorkOptimization;
    ...
    public class Box : IDataErrorInfo {
        private const double _gamma = 3.14;
        public double X1 = 0.7;
        ...
        public List<Point> RandomSearch() {
            ...
            if (!Check(X1, X2)) {
                throw new Exception("Стартовая точка за пределами ограничений!");
            }
        ...
        public List<Point> BoxAlgorithm() {
            if (!Check(X1, X2)) {
                throw new Exception("Стартовая точка за пределами ограничений!");
            }
            List<Point> result = new List<Point>();
            result.Add(new Point(X1, X2, this));
            ...
            while (fixedVertices.Count < vertexQuantity) {
                ...
                while (!Check(wrongVertices[0].X1, wrongVertices[0].X2)) {
